...
if (isset($_POST['simpan_pulang'])) {
    ...
    $stmt = $pdo->prepare("INSERT INTO log_izin_siswa (tanggal, extinction, nama, kelas, keterangan, tipe_izin) VALUES (?, ?, ?, ?, ?, ?)");
    // Catatan: Jika di database kolom jam kamu namanya 'waktu', ubah 'extinction' di atas menjadi 'waktu' sesuai database.
    ...
    if ($stmt->execute([$tanggal, $waktu, $nama, $kelas, $keterangan_full, $tipe_izin])) {
        echo "<script>alert('Data izin pulang siswa berhasil disimpan!'); window.location.href='index.php';</script>";
        exit();
    }
}
...
<div style="max-width: 550px; margin: 40px auto; padding: 30px; background: white; border-radius: 12px; box-shadow: 0 8px 20px rgba(0,0,0,0.05); font-family: 'Segoe UI', sans-serif; border-top: 5px solid #22c55e;">
    
    <div style="text-align: center; margin-bottom: 25px;">
        <h3 style="color: #1e3a8a; margin: 0; font-size: 1.4rem; font-weight: bold; letter-spacing: 0.5px;">🏠 INPUT SISWA IZIN PULANG</h3>
    </div>
    ...
    <form method="POST" action="">
        ...
        <div style="display: flex; gap: 15px; margin-bottom: 20px;">
            ...
            <div style="flex: 1;">
                <label style="display: block; font-weight: 600; margin-bottom: 6px; color: #475569; font-size: 0.9rem;">Jam Pulang</label>
        ...
        <!-- 4. ALASAN KETERANGAN -->
        <div style="margin-bottom: 20px;">
            <label style="display: block; font-weight: 600; margin-bottom: 6px; color: #475569; font-size: 0.9rem;">Alasan Izin Pulang</label>
            <textarea name="keterangan" rows="3" placeholder="Contoh: Sakit demam, diminta pulang oleh UKS..." required style="width: 100%; padding: 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 1rem; resize: none;"></textarea>
        </div>
        ...
        <div style="display: flex; gap: 12px;">
            <button type="submit" name="simpan_pulang" style="flex: 2; padding: 14px; background: #1e3a8a; color: white; border: none; border-radius: 8px; font-weight: bold; font-size: 1rem; cursor: pointer;">💾 Simpan Data</button>
