...
// Mengambil dari log_izin_siswa dengan filter tipe_izin = 'izin keluar'
$query = "SELECT * FROM log_izin_siswa WHERE tipe_izin = 'izin keluar' AND (nama LIKE '%$search%') ORDER BY tanggal DESC, waktu DESC";
...
<div style="max-width: 1200px; margin: 30px auto; padding: 20px; font-family: 'Segoe UI', sans-serif;">
    <h2 style="color: #10b981; border-bottom: 3px solid #10b981; padding-bottom: 10px;">🟢 Rekapitulasi Izin Keluar</h2>
    ...
    <form method="GET" style="margin: 20px 0; display: flex; gap: 10px;">
        ...
        <button type="submit" style="padding: 10px 20px; background: #10b981; color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: bold;">Cari</button>
    </form>
    ...
    <table style="width: 100%; border-collapse: collapse; background: white; box-shadow: 0 4px 6px rgba(0,0,0,0.1); border-radius: 8px; overflow: hidden;">
        <thead>
            <tr style="background: #10b981; color: white; text-align: left;">
                ...
                <th style="padding: 15px; border: 1px solid #ddd;">Keperluan / Keterangan</th>
            </tr>
        </thead>
        <tbody>
            <?php if ($result && mysqli_num_rows($result) > 0): ?>
                ...
                <tr style="border-bottom: 1px solid #eee;">
                    ...
                    <td style="padding: 12px; border: 1px solid #ddd;"><?= $row['keterangan'] ?></td> </tr>
                <?php endwhile; ?>
            <?php else: ?>
                <tr>
                    <td colspan="5" style="padding: 20px; text-align: center; color: #64748b;">Tidak ada data log izin keluar.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
