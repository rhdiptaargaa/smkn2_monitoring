...
// Mengambil dari log_izin_siswa dengan filter tipe_izin = 'izin pulang'
$query = "SELECT * FROM log_izin_siswa WHERE tipe_izin = 'izin pulang' AND (nama LIKE '%$search%') ORDER BY tanggal DESC, waktu DESC";
...
<div style="max-width: 1200px; margin: 30px auto; padding: 20px; font-family: 'Segoe UI', sans-serif;">
    <h2 style="color: #f97316; border-bottom: 3px solid #f97316; padding-bottom: 10px;">🟠 Rekapitulasi Izin Pulang</h2>
    ...
    <form method="GET" style="margin: 20px 0; display: flex; gap: 10px;">
        ...
        <button type="submit" style="padding: 10px 20px; background: #f97316; color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: bold;">Cari</button>
    </form>
    ...
    <table style="width: 100%; border-collapse: collapse; background: white; box-shadow: 0 4px 6px rgba(0,0,0,0.1); border-radius: 8px; overflow: hidden;">
        <thead>
            <tr style="background: #f97316; color: white; text-align: left;">
                ...
                <th style="padding: 15px; border: 1px solid #ddd;">Keterangan Izin</th>
            </tr>
        </thead>
        <tbody>
            ...
            <?php else: ?>
                <tr>
                    <td colspan="5" style="padding: 20px; text-align: center; color: #64748b;">Tidak ada data log izin pulang.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
